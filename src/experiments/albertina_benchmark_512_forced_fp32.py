...
DEFAULT_RESULTS_PATH = PROJECT_ROOT / "results" / "albertina_benchmark_512_forced_fp32.csv"
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "checkpoints" / "albertina_benchmark_512_forced_fp32"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--history-output",
        type=Path,
        default=PROJECT_ROOT / "results" / "albertina_benchmark_512_forced_fp32_history.csv",
    )
...
def main() -> int:
    ...
    if getattr(args, "bf16", False) or getattr(args, "no_fp16", False):
        print(
            "Aviso: este script dedicado força FP32; flags de precisão "
            "são ignoradas."
        )

    # DeBERTa possui um problema conhecido no caminho mixed-precision
    # (FP16/BF16) em DisentangledSelfAttention.masked_fill no Transformers
    # usado neste projeto. Este benchmark diagnostico roda deliberadamente
    # em FP32 para evitar o overflow antes do primeiro step.
    bf16 = False
    fp16 = False
    ...
    print("=" * 72)
    print("ALBERTINA 100M PT-BR — BENCHMARK FORCED FP32 (GAS FIX)")
    print("=" * 72)
    ...
    model = AutoModelForSequenceClassification.from_pretrained(
        ...
        # Transformers 5.x respeita por padrao o dtype salvo no checkpoint.
        # A Albertina foi salva/configurada como BF16, mas a implementacao
        # DeBERTa desta versao mistura tensores Float/BFloat16 em certas
        # operacoes. Portanto forçamos TODO o modelo para FP32 no load.
        dtype=torch.float32,
    )

    print(f"classifier_pooling        : {getattr(model.config, 'classifier_pooling', None)}")
    print(f"problem_type              : {model.config.problem_type}")
    print(f"config dtype              : {getattr(model.config, 'dtype', None)}")
    ...
    if floating_dtypes != ["torch.float32"]:
        raise RuntimeError(
            "O modelo nao foi carregado integralmente em FP32. "
