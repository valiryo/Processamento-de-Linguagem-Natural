...
DEFAULT_SUMMARY_OUTPUT = PROJECT_ROOT / "results" / "norberto_tokenization_summary.csv"
DEFAULT_LENGTHS_OUTPUT = PROJECT_ROOT / "results" / "norberto_token_lengths.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Analisa a tokenizacao do Itau-Unibanco/NorBERTo-base no train.xlsx "
...
def load_dataset(path: Path) -> pd.DataFrame:
    ...
    unexpected_classes = sorted(set(df["clarity"].unique()) - set(EXPECTED_CLASSES))
    if unexpected_classes:
        raise ValueError(
            "Foram encontradas classes inesperadas em 'clarity': "
            f"{unexpected_classes}. Esperado: {list(EXPECTED_CLASSES)}"
        )

    # row_id aqui corresponde exatamente à posição zero-based da linha no DataFrame
    # carregado de train.xlsx. Nenhum fold é criado ou alterado por este script.
...
def load_tokenizer():
    ...
    print(f"model_max_length reportado: {tokenizer.model_max_length}")
...
def compute_token_lengths(
    ...
    # add_special_tokens=True é intencional:
    # max_length limita a sequência final entregue ao modelo, incluindo tokens especiais.
...
def length_statistics(lengths: pd.Series) -> dict[str, float | int]:
    if lengths.empty:
        raise ValueError("Nao e possivel calcular estatisticas de um subconjunto vazio.")
...
def build_summary(
    ...
    for scope, class_name, subset in groups:
        ...
        for max_length in MAX_LENGTHS:
            ...
            discarded = (lengths.loc[truncated_mask] - max_length).clip(lower=0)
            ...
            rows.append(
                {
                    ...
                    "discarded_tokens_mean_truncated": (
                        float(discarded.mean()) if truncated_count > 0 else 0.0
                    ),
                    **stats,
                }
            )
...
def print_console_summary(summary: pd.DataFrame) -> None:
    ...
    for column in (
        ...
        printable[column] = printable[column].map(lambda x: round(float(x), 4))

    print("\nResumo de cobertura/truncamento:")
    print(printable.to_string(index=False))

    overall_first = summary[
        (summary["scope"] == "overall") & (summary["max_length"] == MAX_LENGTHS[0])
    ].iloc[0]

    print("\nEstatisticas globais do comprimento tokenizado (com tokens especiais):")
    print(f"  media   : {overall_first['n_tokens_mean']:.2f}")
    print(f"  mediana : {overall_first['n_tokens_median']:.2f}")
    print(f"  p75     : {overall_first['n_tokens_p75']:.2f}")
    print(f"  p90     : {overall_first['n_tokens_p90']:.2f}")
    print(f"  p95     : {overall_first['n_tokens_p95']:.2f}")
    print(f"  p99     : {overall_first['n_tokens_p99']:.2f}")
    print(f"  maximo  : {int(overall_first['n_tokens_max'])}")
...
def main() -> int:
    ...
    print(df["clarity"].value_counts().reindex(EXPECTED_CLASSES).to_string())

    tokenizer = load_tokenizer()

    print(
        "\nCalculando comprimento SEM truncamento para todas as instancias "
    ...
    lengths_output = df[["row_id", "clarity", "n_tokens"]].copy()

    summary = build_summary(df=df, tokenizer=tokenizer)
    ...
    lengths_output.to_csv(args.lengths_output, index=False, encoding="utf-8-sig")
    ...
    print(
        "\nConcluido. Este script nao treina modelo, nao carrega folds "
        "e nao acessa test.xlsx."
    )
