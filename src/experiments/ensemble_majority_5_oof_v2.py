...
CV_OUT = RESULTS / "ensemble_majority_5_v2_cv.csv"
OOF_OUT = RESULTS / "ensemble_majority_5_v2_oof.csv"
SUMMARY_OUT = RESULTS / "ensemble_majority_5_v2_summary.csv"
...
def majority_vote(predictions: np.ndarray) -> np.ndarray:
    ...
    Regra:
    1. escolhe a classe com maior número de votos;
    2. no caso 2-2-1, o desempate fica RESTRITO às duas classes
       empatadas na liderança;
    3. entre elas, usa a seguinte hierarquia fixa de modelos:
       BERT512 -> soft256 -> SVC -> baseline -> ordinal.

    A restrição do desempate é importante: a versão anterior podia escolher
    a previsão do BERT512 mesmo quando ela era a classe com apenas 1 voto.
    """
    out = np.empty(
        predictions.shape[0],
        dtype=int,
    ...
    for i, row in enumerate(predictions):
        ...
        # Desempate determinístico, mas somente entre as classes líderes.
        chosen = None

        for model_idx in range(predictions.shape[1]):
            candidate = int(row[model_idx])
        ...
        if chosen is None:
            raise RuntimeError(
                "Falha inesperada no desempate do voto majoritário."
            )
